...
const LandingPage: React.FC<LandingPageProps> = ({ 
  ...
  React.useEffect(() => {
    const fetchCompanies = async () => {
      try {
        // Fetch more initially to filter client-side if needed, but limit to 5 for display
        ...
        const publicCompanies = (data || []).filter(c => c.branding?.isProfilePublic !== false).slice(0, 5);
  ...
  const slideshowPartners = branding.globalPartners && branding.globalPartners.length > 0 
    ...
    : companies.map(c => ({
        id: c.id,
        name: c.name,
        logoUrl: c.branding?.agencyLogo || '', // Use agencyLogo if present, otherwise fallback to empty string for icon rendering
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {slideshowPartners.slice(0, 10).map((partner, idx) => (
              <a 
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="bg-white p-6 rounded-2xl border border-safari-100 hover:border-safari-900/20 hover:shadow-xl hover:shadow-safari-900/5 transition-all group/card flex flex-col items-center justify-center text-center space-y-4 cursor-pointer"
              >
