...
import { Compass, Settings, AlertTriangle, Calculator, Loader2, LogOut, Menu, X, Home, User, Briefcase, Calculator as CalcIcon, Download, Sparkles } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'landing' | 'form' | 'itinerary' | 'history' | 'admin' | 'calculator' | 'auth' | 'partners' | 'subscription' | 'success' | 'cancel' | 'reset-password' | 'not-found' | 'company-profile'>(() => {
  ...
  // PWA Install State & Hook listeners
  ...
  useEffect(() => {
    ...
    const handleBeforeInstallPrompt = (e: Event) => {
      ...
      const dismissed = sessionStorage.getItem('safari_pwa_dismissed');
      if (!dismissed) {
  ...
  const handleDismissPwaBanner = () => {
    setShowPwaBanner(false);
    sessionStorage.setItem('safari_pwa_dismissed', 'true');
  };
  ...
  return (
    <>
      ...
      {showPwaBanner && isInstallable && !isAppInstalled && (
        <div className="fixed bottom-6 right-6 left-6 md:left-auto md:w-96 bg-white border border-safari-200 shadow-2xl p-5 rounded-2xl z-[999] animate-fadeIn flex flex-col gap-3">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-safari-600 to-safari-800 text-white flex items-center justify-center shrink-0 shadow-md">
              <Compass size={24} className="text-white animate-spin-slow" />
            </div>
            <div className="flex-1 min-w-0">
              <h4 className="text-sm font-bold text-safari-900 flex items-center gap-1.5 leading-tight animate-pulse">
                Install SafariPlanner App <Sparkles size={14} className="text-amber-500 fill-amber-500" />
              </h4>
              <p className="text-xs text-safari-500 mt-1.5 leading-relaxed">
                Save your bespoke African itineraries and access professional costing tools seamlessly offline.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3 mt-2 w-full">
            <button
              ...
              className="flex-1 py-2 text-xs font-bold text-safari-600 hover:text-safari-900 hover:bg-safari-50 rounded-lg transition-colors border border-safari-200"
            >
              Maybe Later
            </button>
            <button
              onClick={handleInstallApp}
              className="flex-1 py-2 text-xs font-bold text-white bg-safari-600 hover:bg-safari-700 rounded-lg shadow-md transition-all flex items-center justify-center gap-1.5"
            >
              <Download size={14} />
              Install Now
            </button>
